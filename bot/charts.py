...
def build_trends_chart(
    *,
    ...
    """
    2-panel grouped bar chart:
      Left  → Weekly  comparison (last week vs this week)
      Right → Monthly comparison (last month vs this month)
    All cent values are divided by 100 for display.
    """
    ...
    fig, (ax_w, ax_m) = plt.subplots(
        1, 2, figsize=(11, 5), facecolor=_BG
    )
    ...
    for ax, title, labels, inc_vals, exp_vals in [
        (
            ax_w,
            "Weekly Trend",
            ["Last Week", "This Week"],
        ...
        bars_i = ax.bar(
            x - width / 2, inc_vals, width,
            color=_INCOME, alpha=0.92, label="Income", zorder=3,
        )
        bars_e = ax.bar(
            x + width / 2, exp_vals, width,
            color=_EXPENSE, alpha=0.92, label="Expense", zorder=3,
        )

        _style_ax(ax, title, currency)
        ax.set_xticks(x)
        ax.set_xticklabels(labels, color=_LABEL)
        _bar_labels(ax, bars_i, currency)
        _bar_labels(ax, bars_e, currency)
